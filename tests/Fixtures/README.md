...
- `torsion.csv`: a1,a2,a3,a4,a6; group structure; all rational torsion points
  (`O` for infinity, otherwise `x:y`, separated by semicolons). Generated by
  `generate-torsion.gp` using `elltors` and exact group operations, covering all
  15 possible groups over Q, rational coordinate changes, nearby decimal
  coefficients and small random general models. Regenerate with
  `gp -q -f tests/Fixtures/generate-torsion.gp > tests/Fixtures/torsion.csv`.
