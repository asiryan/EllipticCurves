...
def run(args):
    ...
    module_names=['blind_search.py','point_search.py','bounded_anchor_pool.py','anchor_diversity.py','point_arithmetic.py']
        ...
        with ThreadPoolExecutor(max_workers=args.workers) as executor:
            while time.perf_counter()<deadline and cert['LowerBound']<args.target:
                ...
                if (folder/'models.json').exists():models=json.loads((folder/'models.json').read_text())
                    ...
                    else:pool=generate(folder/'basis.json',folder/'pool',args.anchors,args.anchors,min(60,max(1,deadline-time.perf_counter())),selector=diverse_vectors)
                ...
                for n,d in boxes():
                    ...
                    for offset in range(0,len(candidates),POLICY['batch_size']):
                        ...
                        # Each worker already checks its own points. Normalize
                        # the complete archive once per batch, not 32 times.
                        if batch_points:
                            found=clean({'ainvs':found['ainvs'],'points':found['points']+batch_points})
